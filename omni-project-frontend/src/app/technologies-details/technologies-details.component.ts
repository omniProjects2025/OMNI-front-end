...
export class TechnologiesDetailsComponent {
  ...
  activatedRoutesData() {
    this.activated_routes.queryParams.subscribe(params => {
      ...
      const selectedTitle = params['selected_tech']?.trim().toLowerCase();
      if (selectedTitle) {
        this.gettingTechnologies = this.technologies.find(
          tech => tech.title.trim().toLowerCase() === selectedTitle
        );
        console.log(this.gettingTechnologies, 'matched technology...');
      } else {
        this.gettingTechnologies = null;
      }
    });
  }
}
